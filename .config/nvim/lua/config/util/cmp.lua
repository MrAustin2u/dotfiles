...
---@alias aa.cmp.Action fun():boolean?
---@type table<string, aa.cmp.Action>
M.actions = {
  -- Native Snippets
...
function M.auto_brackets(entry)
  local cmp = require ".config.nvim.lua.config.util.cmp"
...
function M.setup(opts)
  ...
  require("cmp.utils.snippet").parse = function(input)
    ...
    return aa.cmp.snippet_preview(input)
  end
  ...
  cmp.event:on("confirm_done", function(event)
    if vim.tbl_contains(opts.auto_brackets or {}, vim.bo.filetype) then
      aa.cmp.auto_brackets(event.entry)
    end
  end)
  cmp.event:on("menu_opened", function(event)
    aa.cmp.add_missing_snippet_docs(event.window)
  end)
end
