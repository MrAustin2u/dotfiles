...
function M.config()
  ...
  require("cokeline").setup({
    ...
    components = {
      ...
      {
        text = function(buffer)
          return (buffer.diagnostics.errors ~= 0 and " " .. icons.diagnostics.Question .. " " .. buffer.diagnostics.errors)
              or (buffer.diagnostics.warnings ~= 0 and " " .. icons.diagnostics.BoldWarning .. " " .. buffer.diagnostics.warnings)
              or ""
        end,
        fg = function(buffer)
      ...
      {
        text = function(buffer)
          return buffer.is_modified and "●" .. " " or icons.ui.Close .. " "
        end,
        fg = function(buffer)
